class ApplicationController < ActionController::Base
 ...
 private
  ...
  def free_canada_shipping?(order)
    @order = order
    @order.delivery_country.name.downcase == "canada" && @order.total_before_tax - @order.delivery_price >= 100
  end

  def free_international_shipping?(order)
    @order = order
    @order.delivery_country.name.downcase != "canada" && @order.total_before_tax - @order.delivery_price >= 175
  end
